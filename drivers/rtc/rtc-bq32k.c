...
#define BQ32K_CAL_CFG1		0x07	/* Calibration register address */

#define BQ32K_SF_KEY1		0x20	/* Calibration register address */
#define BQ32K_SF_KEY2		0x21	/* Calibration register address */
#define BQ32K_SFR		0x22	/* Calibration register address */
#define BQ32K_TCH2		0x08	/* Calibration register address */
#define BQ32K_CFG2		0x09	/* Calibration register address */
				...
				const struct i2c_device_id *id)
{
	...
	reg = 0x80;
	error = bq32k_write(dev, &reg, BQ32K_CAL_CFG1, 1);
	reg = 0x5E;
	error = bq32k_write(dev, &reg, BQ32K_SF_KEY1, 1);
	reg = 0xC7;
	error = bq32k_write(dev, &reg, BQ32K_SF_KEY2, 1);
	reg = 0x00;
	error = bq32k_write(dev, &reg, BQ32K_SFR, 1);
	reg = 0x20;	//0x20 attivo TCH2 0x00 spegne
	error = bq32k_write(dev, &reg, BQ32K_TCH2, 1);
	reg = 0x85;	//0xC5 attivo TCFE e TCHE 0x85 spegne TCFE
